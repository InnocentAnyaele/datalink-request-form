...
import ViewDefermentModal from './ViewDefermentModal';
import DeleteDeferment from './DeleteDeferment';
import DefermentApproval from './DefermentApproval';

function ApproveDeferment() {
	...
	useEffect(() => {
		if (query !== '') {
			axios.get(`/deferment/searchDefermentFalse/${query}`).then((res) => {
				setItems(res.data);
				setLoading(false);
			});
		} else {
			axios
				.get(`/deferment/getDefermentFalse/${query}`)
				.then((res) => {
					setItems(res.data);
					setLoading(false);
				})
...
export default ApproveDeferment;
