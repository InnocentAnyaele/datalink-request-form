...
import CompleteDeferment from './CompleteDeferment';

function DefermentApproved() {
	...
	useEffect(() => {
		if (query !== '') {
			axios.get(`/deferment/searchDefermentTrue/${query}`).then((res) => {
				setItems(res.data);
				setLoading(false);
			});
		} else {
			axios
				.get(`/deferment/getDefermentTrue/${query}`)
				.then((res) => {
					setItems(res.data);
					setLoading(false);
				})
...
export default DefermentApproved;
