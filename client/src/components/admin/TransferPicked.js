...
function TransferPicked() {
	...
	useEffect(() => {
		if (query !== '') {
			axios.get(`/transfer/searchPickedTransfer/${query}`).then((res) => {
				setItems(res.data);
				setLoading(false);
			});
		} else {
			axios
				.get(`/transfer/getPickedTransfer`)
				.then((res) => {
					setItems(res.data);
					setLoading(false);
				})
