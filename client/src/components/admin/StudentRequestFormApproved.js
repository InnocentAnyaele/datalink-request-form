...
function StudentRequestFormPending() {
	...
	useEffect(() => {
		if (query !== '') {
			axios.get(`/studentRequest/searchPaidTrue/${query}`).then((res) => {
				setItems(res.data);
				setLoading(false);
			});
		} else {
			axios
				.get('/studentRequest/getPaidTrue')
				.then((res) => {
					setItems(res.data);
					setLoading(false);
				})
...
export default StudentRequestFormPending;
