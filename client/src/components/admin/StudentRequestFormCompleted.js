...
import PickedStudentRequest from './PickedStudentRequest';

function StudentRequestFormCompleted() {
	...
	useEffect(() => {
		if (query !== '') {
			axios
				.get(`/studentRequest/searchCompletedRequest/${query}`)
				.then((res) => {
					setItems(res.data);
					setLoading(false);
				});
			...
			axios
				.get('/studentRequest/getCompleteRequest')
				.then((res) => {
					setItems(res.data);
					setLoading(false);
				})
...
export default StudentRequestFormCompleted;
